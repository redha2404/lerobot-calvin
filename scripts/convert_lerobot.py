...
- **Tasks Count:** {len(selected_tasks)}
...
def main():
    parser = argparse.ArgumentParser(description="Convert CALVIN datasets to LeRobot format")
    ...
    parser.add_argument("--tasks", nargs="+", required=True, 
                        help="List of task names/annotations to convert. Wrap each in quotes, e.g., 'pick up the pink block' 'push blue block'")
    args = parser.parse_args()

    train_dir = str(Path(args.dataset_path) / "training")
    val_dir = str(Path(args.dataset_path) / "validation")
